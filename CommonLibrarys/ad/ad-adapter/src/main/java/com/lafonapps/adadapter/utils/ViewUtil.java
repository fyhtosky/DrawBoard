...
public class ViewUtil {
    ...
    public static int getDeviceWidthInDP(Context context) {
        return px2dp(getDeviceWidthInPx(context),context);
    }
    ...
    public static int getDeviceHeightInDP(Context context) {
        return px2dp(getDeviceHeightInPx(context),context);
    }
...
//      return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
     ...
     * @return
     */
    public static float distanceInDp(float x1, float y1, float x2, float y2,Context context) {
        float distanceInPx = distanceInPx(x1, y1, x2, y2);
        return ViewUtil.px2dp(distanceInPx,context);
    }
